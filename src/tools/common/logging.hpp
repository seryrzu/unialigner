...
class TimeSpace {
 ...
 public:
    ...
    std::string get() const {
        ...
        auto worktime = size_t(double(finish.tv_sec - start.tv_sec)
...
class LoggerStorage {
 ...
 public:
    explicit LoggerStorage(std::experimental::filesystem::path _dir,
    ...
    std::experimental::filesystem::path backup() const {
        ...
        size_t max = 0;
        for (const std::experimental::filesystem::path
                &file : std::experimental::filesystem::directory_iterator(
            backupDir)) {
            std::string fname = file.filename().string();
            if (fname.size() < 5 || fname.substr(fname.size() - 4)!=".log")
                continue;
            try {
                max = std::max<size_t>(max,
                                       std::stoi(fname.substr(0,
                                                              fname.size()
                                                                  - 4)));
        ...
        std::experimental::filesystem::path
            backup = backupDir/(itos(max + 1, 2) + ".log");
...
class ProgressBar {
 ...
 public:
    ProgressBar(Logger &logger,
                size_t size,
                size_t threads,
                size_t bar_num = 20) : logger(logger),
                                       cur(1),
                                       size(size),
                                       bar_num(std::min(bar_num, size)),
                                       mod(std::max<size_t>(1,
                                                            size/bar_num/threads
                                                                /5)),
                                       progress(threads),
                                       cnt(0) {
...
inline void logGit(Logger &logger,
                   const std::experimental::filesystem::path &out) {
    int code = system(("bash -c \"git rev-parse HEAD;git diff\" 2> /dev/null > "
        + out.string()).c_str());
    std::ifstream is;
    is.open(out);
    std::string res;
